...
contract Escrow is Ownable {
    ...
    function getEscrowByWithdrawer(address _withdrawer) view external returns (uint256[] memory) {
        return depositsByWithdrawers[_withdrawer];
    }

    function getEscrowById(uint256 _id) view external returns (Items memory) {
